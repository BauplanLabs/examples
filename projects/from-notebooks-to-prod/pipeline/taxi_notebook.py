...
def compute_stats_by_zone(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.filter(
        ...
        # Create a new column with log-transformed
        # trip_miles to better model skewed
        # distribution.
        pl.col("trip_miles").log10().alias("log_trip_miles")
    )
    result = (
        df.select(["Zone", "log_trip_miles"])
        .group_by("Zone")
        .agg(pl.col("log_trip_miles").median().alias("log_trip_miles"))
    )
